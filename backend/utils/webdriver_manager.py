...
def get_chrome_exe_path():
    paths = [
        pjoin(CD, 'Program Files', 'Google', 'Chrome', 'Application'),
        pjoin(CD, 'Program Files (x86)', 'Google', 'Chrome', 'Application'),
        pjoin(CHROME, 'Application')
    ]
...
def get_chrome_driver(update=False):
...
def get_opera_driver(update=False, gx=False):
    path = pjoin(OPERA, 'operadriver_win32', 'operadriver.exe')
    if not update and exists(path):
        return path
    ov = get_opera_version(gx)
...
def get_yandex_driver(update=False):
    path = pjoin(YANDEX, 'yandexdriver.exe')
    if not update and exists(path):
        return path
    yav = get_yandex_version()
...
def get_firefox_driver(update=False):
...
def get_edge_driver(update=False):
...
def get_phantomjs_driver(update=False):
